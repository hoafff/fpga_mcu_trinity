...
class SystemInfo:
    ...
    def decode(cls, payload: bytes) -> "SystemInfo":
        ...
        if payload[3] != 0:
            raise HostProtocolError("system info reserved byte is non-zero")
        capabilities, sn32_build, p1_build, p2_build = struct.unpack_from(">IIII", payload, 4)
        return cls(payload[0], payload[1], payload[2], capabilities,
...
class TrinitySerialClient:
    ...
    def get_system_info(self) -> SystemInfo:
        return SystemInfo.decode(self.request(HostCommand.GET_SYSTEM_INFO).payload)

    def get_system_status(self) -> SystemStatus:
        return SystemStatus.decode(self.request(HostCommand.GET_SYSTEM_STATUS).payload)
    ...
    def start_p1_self_test(
        ...
        *,
        ...
        test_mask: int = int(
            TestMask.MEMORY | TestMask.NTT | TestMask.INTT | TestMask.BASEMUL | TestMask.ASCON
        ),
    ) -> int:
    ...
    def run_p1_bringup(
        ...
        *,
        ...
        if result is None or result.state not in {
            ...
            raise TimeoutError(f"P1 self-test transaction 0x{host_txid:04X} did not finish")
        if result.queried_txid != host_txid or result.original_command != int(HostCommand.RUN_SELF_TEST):
            raise HostProtocolError("retained transaction correlation mismatch")
        if result.state != TransactionState.SUCCEEDED or result.result_code != ErrorCode.OK:
            raise HostProtocolError(
                f"P1 self-test failed: state={result.state.name}, code={result.result_code.name}"
            )
