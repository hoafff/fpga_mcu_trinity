module ascon_aead128_decrypt (
    ...
    input  logic [191:0] ciphertext_i,
    input  logic [127:0] tag_i,
    output logic         busy_o,
    output logic         done_o,
    output logic         tag_valid_o,
    output logic [191:0] plaintext_o
);
    ...
    if (!rst_ni) begin
      ...
      case (state)
        ...
        S_DOMAIN_SEP: begin
          ...
          plaintext_o[63:0] <= x0 ^ ct_reg[63:0];
          plaintext_o[127:64] <= x1 ^ ct_reg[127:64];
          x0 <= ct_reg[63:0];
          x1 <= ct_reg[127:64];
          round_index <= 4'd4;
          state <= S_MSG0_P8;
        ...
        S_MSG1: begin
          plaintext_o[191:128] <= x0 ^ ct_reg[191:128];
          x0 <= ct_reg[191:128];
        ...
        S_TAG: begin
          tag_valid_o <= (((x3 ^ k0) == tag_reg[63:0]) &&
                          ((x4 ^ k1) == tag_reg[127:64]));
          state <= S_DONE;
        end
        ...
        default: begin
          // Illegal control state is treated as a fatal local condition: return
          // to idle only after clearing every secret-bearing register and all
          // externally visible result state.
          state <= S_IDLE;
          busy_o <= 1'b0;
          done_o <= 1'b0;
          tag_valid_o <= 1'b0;
