...
logic secure,zeroize,fault; logic [15:0] code; logic [31:0] fault_time; logic [2:0] state,hb_timeout,hb_seen; logic all_healthy;
fpst_supervisor_core #(.HB_TIMEOUT_MS(8),.STARTUP_ZEROIZE_MS(2),.STARTUP_GRACE_MS(4),.QUALIFY_MS(6),.ZEROIZE_HOLD_MS(2),.RECOVERY_QUALIFY_MS(6)) dut(
 ...
 .secure_enable_o(secure),.key_zeroize_o(zeroize),.fault_latched_o(fault),
...
task automatic reset_dut; begin
 rst_n=0; hb_mcu=0; hb_pqc=0; hb_crypto=0; crypto_fault=0; tamper=0; clear_pulse=0; manual_fault=0;
 repeat(4) @(posedge clk);
 #1; if(secure!==0||zeroize!==1) $fatal(1,"reset is not fail-safe");
...
initial begin
 ...
 reset_dut(); boot(); tamper=1; repeat(2) @(posedge clk);
 ...
 reset_dut(); boot(); force dut.state_q=3'b100; @(posedge clk); release dut.state_q; repeat(2) @(posedge clk);
 if(!fault||code!==E_ILLEGAL||secure) $fatal(1,"retired reset-state fail-safe failed");
