module kiwi_primer20k_ascon_selftest_top #(
    ...
    ascon_encrypt_kat_selftest u_selftest (
        .clk_i                 (sys_clk_i),
    ...
    assign led6_no = ~core_busy;
    assign led7_no = ~(core_error_code != 16'h0000);

`ifndef SYNTHESIS
    logic unused_debug;
    always_comb begin
        unused_debug = selftest_done ^ (^mismatch_index) ^
                       (^mismatch_observed) ^ (^mismatch_expected);
    end
`endif
endmodule
