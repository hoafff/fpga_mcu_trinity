module primer2_session_context (
    input  logic         clk_i,
    input  logic         rst_ni,
    ...
    always_ff @(posedge clk_i) begin
        if (!rst_ni) begin
            ...
            if (load_begin_i) begin
                /* Loading a new RX context invalidates the old session immediately. */
                key_loading_o        <= 1'b1;
                key_valid_o          <= 1'b0;
                session_active_o     <= 1'b0;
                session_id_o         <= 32'h0;
            ...
            if (load_chunk_i && key_loading_o && (load_offset_i < CONTEXT_BYTES)) begin
                if (!coverage_q[load_offset_i]) begin
                    ...
                    /* Exact retransmission is harmless; conflicting bytes are not. */
            ...
            /* Atomic RX replay-state commit occurs only after authenticated release. */
            if (sequence_commit_i && session_active_o)
                expected_sequence_o <= expected_sequence_o + 1'b1;

            /* Highest priority: secret/replay context is invalid in this clock. */
            if (zeroize_i) begin
                key_loading_o        <= 1'b0;
                key_valid_o          <= 1'b0;
                session_active_o     <= 1'b0;
