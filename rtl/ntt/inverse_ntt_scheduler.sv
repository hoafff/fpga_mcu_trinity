module inverse_ntt_scheduler (
    ...
    // FIPS 203 Algorithm 10 schedule (before the final multiplication by 3303):
    //
    //   i = 127
    ...
    //         inverse_butterfly(j, j + len, zeta);
    //     }
    //
    // Exactly 896 butterfly transactions are emitted. State advances only on
    // valid_o && ready_i.
    ...
    always_comb begin
        ...
        next_group_start = {1'b0,start_q} + ({1'b0,length_q} << 1);
        right_addr_ext = {1'b0,left_q} + {1'b0,length_q};
        current_group_last = ({1'b0,left_q} + 9'd1) == group_end_exclusive;
    ...
    always_comb begin
        ...
        if (busy_q) begin
            ...
            transform_first_o = (stage_q == 3'd0) && stage_first_o;
            transform_last_o = (stage_q == 3'd6) && current_stage_last;
        end
    end
...
`ifndef SYNTHESIS
    always_ff @(posedge clk_i) begin
        if (rst_ni && busy_q) begin
            assert (stage_q <= 3'd6)
                else $error("inverse_ntt_scheduler: invalid stage %0d", stage_q);
            assert ((length_q == 8'd2) || (length_q == 8'd4) ||
                    (length_q == 8'd8) || (length_q == 8'd16) ||
                    (length_q == 8'd32) || (length_q == 8'd64) ||
                    (length_q == 8'd128))
                else $error("inverse_ntt_scheduler: invalid length %0d", length_q);
            assert (right_addr_ext < N)
                else $error("inverse_ntt_scheduler: right address out of range");
            assert (left_q < right_addr_ext[7:0])
                else $error("inverse_ntt_scheduler: invalid address pair");
            assert ((zeta_q >= 7'd1) && (zeta_q <= 7'd127))
                else $error("inverse_ntt_scheduler: invalid zeta %0d", zeta_q);
        end
    end
`endif
endmodule
