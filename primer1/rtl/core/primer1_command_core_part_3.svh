...
else if ({pbyte(request_payload_i,0),pbyte(request_payload_i,1)}==16'd0 ||
(({pbyte(request_payload_i,0),pbyte(request_payload_i,1)} & ~SELFTEST_SUPPORTED_MASK)!=16'd0))
...
if (({pbyte(request_payload_i,0),pbyte(request_payload_i,1)} & TEST_ASCON)!=0) begin
...
emit_error(request_command_i,request_txid_i,ERR_BAD_LENGTH,0);
else if (pbyte(request_payload_i,0)!=ZEROIZE_ALL)
emit_error(request_command_i,request_txid_i,ERR_NOT_SUPPORTED,pbyte(request_payload_i,0));
else begin
begin_retained(request_txid_i,request_command_i,incoming_fingerprint);
...
emit_error(request_command_i,request_txid_i,ERR_BAD_LENGTH,0);
else if ({pbyte(request_payload_i,0),pbyte(request_payload_i,1),
pbyte(request_payload_i,2),pbyte(request_payload_i,3)}!=32'd0 &&
!(staged_valid && staged_session_id=={pbyte(request_payload_i,0),pbyte(request_payload_i,1),
pbyte(request_payload_i,2),pbyte(request_payload_i,3)}) &&
!(active_valid && active_session_id=={pbyte(request_payload_i,0),pbyte(request_payload_i,1),
pbyte(request_payload_i,2),pbyte(request_payload_i,3)}))
emit_error(request_command_i,request_txid_i,ERR_BAD_SESSION,0);
else begin
staged_key<=0;active_key<=0;staged_nonce_prefix<=0;active_nonce_prefix<=0;
...
emit_error(request_command_i,request_txid_i,ERR_RESULT_PENDING,0);
else if (operation_state!=OP_IDLE)
emit_error(request_command_i,request_txid_i,ERR_BAD_STATE,0);
