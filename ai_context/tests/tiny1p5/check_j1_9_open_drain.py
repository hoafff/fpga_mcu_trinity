...
TOP = ROOT / "tiny1p5/rtl/supervisor_top.sv"
CORE = ROOT / "tiny1p5/rtl/fpst_supervisor_core.sv"
CST = ROOT / "tiny1p5/constraints/kiwi_tiny1p5_fpst.cst"
...
for required in ("IO_TYPE=LVCMOS33", "OPEN_DRAIN=ON", "PULL_MODE=NONE"):
    if required not in port_lines[0]:
        fail(f"Tiny_FAULT_N constraint is missing {required}")
if "DRIVE=" in port_lines[0] or "PULL_MODE=UP" in port_lines[0]:
    fail("Tiny_FAULT_N constraint enables a forbidden drive/pull attribute")
print("PASS: relocated Tiny J1-9 static contract (single 0/Z driver, open drain, no pull)")
